...
if CLIENT then
    ...
    hook.Add("Initialize", "TTTInitMarkerGunLang", function()
        ...
        LANG.AddToLanguage(
            "Deutsch",
            "ttt2_paintgun_desc",
            "Schieße auf einen Spieler, um ihn zu markieren."
        )
    end)
end
...
function SWEP:PrimaryAttack()
    ...
    if SERVER then
        ...
        shoot_pos = shoot_pos + owner:GetForward() * 10
        shoot_pos = shoot_pos + owner:GetRight() * 7
        shoot_pos = shoot_pos + owner:GetUp() * -0.75

        local shoot_vector = owner:GetEyeTrace().HitPos - shoot_pos
...
if SERVER then
    hook.Add("ScalePlayerDamage", "MarkerHitReg", function(ply, hitgroup, dmginfo)
        ...
        if wep:GetClass() ~= "weapon_ttt2_markergun" then
            return
        end
