...
hook.Add("TTTRenderEntityInfo", "ttt2_marker_highlight_focused_players", function(tData)
    ...
    if LocalPlayer():GetSubRole() ~= ROLE_MARKER then
        return
    end

    if
        tData:GetAmountDescriptionLines() > 0
    ...
    if MARKER_DATA:IsMarked(ent) then
        tData:AddDescriptionLine(LANG.GetTranslation("ttt_marker_player_marked"), MARKER.ltcolor)

        tData:AddIcon(MARKER.iconMaterial, MARKER.ltcolor)
    ...
    if
        GetGlobalBool("ttt_mark_deal_no_damage", false)
        and not (MARKER_DATA:IsMarked(ent) and GetGlobalBool("ttt_mark_hurt_marked", false))
    then
        tData:AddDescriptionLine(
