...
if CLIENT then
    net.Receive("ttt2_role_marker_new_marking", function()
        ...
        if not marked_player or not marked_player:IsPlayer() then
            return
        end

        local marked_player_id = tostring(marked_player:SteamID64() or marked_player:EntIndex())

        if LocalPlayer():GetTeam() ~= TEAM_MARKER then
            return
        end
    ...
    hook.Add("TTTScoreboardColumns", "ttt2_marked_players_column", function(pnl)
        ...
        if client:GetTeam() ~= TEAM_MARKER then
            return
        end
...
if SERVER then
    ...
    function MARKER_DATA:SetMarkedPlayer(marker, markee, isRevival)
        if not IsValid(markee) or not markee:IsPlayer() or self:IsMarked(markee) then
            return
        end
    ...
    function MARKER_DATA:UpdateAfterChange()
        ...
        self.able_to_win = self:AmountNoMarkerAlive()
            >= math.min(GetConVar("ttt_mark_min_alive"):GetInt(), self.amount_to_win)
    ...
    function MARKER_DATA:MarkerDied()
        if MARKER_DATA:IsMarkerAlive() then
            return
        end
    ...
    hook.Add("PostPlayerDeath", "ttt2_role_marker_death", function(victim, infl, attacker)
        ...
        if victim:GetTeam() ~= TEAM_MARKER then
            return
        end
    ...
    hook.Add("TTT2UpdateTeam", "ttt2_role_marker_team_change", function(ply, old, new)
        if ply:GetSubRole() ~= ROLE_PIRATE and ply:GetSubRole() ~= ROLE_PIRATE_CAPTAIN then
            return
        end
